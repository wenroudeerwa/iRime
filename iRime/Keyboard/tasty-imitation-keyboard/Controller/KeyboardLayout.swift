...
class GlobalColors: NSObject {
    ...
    class var lightModeSolidColorSpecialKey: UIColor { get { return UIColor(red: CGFloat(172)/CGFloat(255), green: CGFloat(178)/CGFloat(255), blue: CGFloat(186)/CGFloat(255), alpha: 1) }}
    ...
    func updateKeyCap(_ key: KeyboardKey, model: Key, fullReset: Bool, uppercase: Bool, characterUppercase: Bool, shiftState: ShiftState) {
        if fullReset {
            ...
            case Key.KeyType.shift:
                if key.shape == nil {
                    let shiftShape = self.getShape(type(of: ShiftShape()))
                    key.shape = shiftShape
                }
            case Key.KeyType.backspace:
                if key.shape == nil {
                    let backspaceShape = self.getShape(type(of: BackspaceShape()))
                    key.shape = backspaceShape
                }
            case Key.KeyType.keyboardChange:
                if key.shape == nil {
                    let globeShape = self.getShape(type(of: GlobeShape()))
